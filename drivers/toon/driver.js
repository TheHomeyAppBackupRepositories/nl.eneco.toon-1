...
class ToonDriver extends OAuth2Driver {
  ...
  onOAuth2Init() {
    ...
    // Deprecated
    const setTemperatureStateActionOld = this.homey.flow.getActionCard('set_temperature_state');
    setTemperatureStateActionOld.registerRunListener(args => args.device.onCapabilityTemperatureStateDeprecated(args.state, (args.resume_program === 'yes')));

    const setTemperatureStateAction = this.homey.flow.getActionCard('set_temperature_state_new');
    setTemperatureStateAction.registerRunListener(args => args.device.onCapabilityTemperatureState(args.state));
